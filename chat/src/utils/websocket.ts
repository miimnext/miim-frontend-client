...
import { updateChatList } from "@/store/ChatListSlice";
...
export const connectWebSocket = (id: string): void => {
  if (!socket || socket.readyState === WebSocket.CLOSED) {
    ...
    socket.onmessage = (event: MessageEvent) => {
      ...
      store.dispatch(updateChatList(chatMessage));
      const callbacks = messageCallbacks.get(chatMessage.conversation_id);
      if (callbacks) {
...
export const subscribeToMessages = (
  ...
  if (!messageCallbacks.has(conversationId)) {
    messageCallbacks.set(conversationId, []);
  }
  const callbacks = messageCallbacks.get(conversationId);
